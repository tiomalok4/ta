...
function selectbase($message){
	...
	$edit = bot("editMessageText",array( "message_id" => $message['message_id'] , "chat_id"=> $chat_id , "text" => "*SELECIONE:*","reply_markup" =>$menu,"reply_to_message_id"=> $message['message_id'],"parse_mode" => 'Markdown'));
	if (!$edit){
		bot("sendMessage",array( "chat_id"=> $chat_id , "text" => "*SELECIONE:*","reply_markup" =>$menu,"reply_to_message_id"=> $message['message_id'],"parse_mode" => 'Markdown'));
	}
}
...
function select($message ,$query, $opt){
	...
	$menu =  ['inline_keyboard' => [
		...
		[['text'=>"Continua compra",'callback_data'=>"ccun"]]

	,]];

	if ($t){
		bot("editMessageText",array( "message_id" => $message['message_id'] , "chat_id"=> $chat_id , "text" => "*Pronto você escolheu: cc's $opt\nPara altera use /country *!" ,"reply_markup" => $menu,"reply_to_message_id"=> $message['message_id'],"parse_mode" => 'Markdown'));
	}
